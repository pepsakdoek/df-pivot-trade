        ...
        widgets.Panel{
            ...
            subviews={
                widgets.HotkeyLabel{
                    ...
                    label='Forbid',
                    key='CUSTOM_F',
                    on_activate=function() self:apply_stock_action('forbid') end,
                    auto_width=true,
                },
                widgets.HotkeyLabel{
                    frame={l=10, t=0},
                    label='Dump',
                    key='CUSTOM_D',
                    on_activate=function() self:apply_stock_action('dump') end,
                    auto_width=true,
                },
                widgets.HotkeyLabel{
                    frame={l=18, t=0},
                    label='Melt',
                    key='CUSTOM_M',
                    on_activate=function() self:apply_stock_action('melt') end,
                    auto_width=true,
                },
                widgets.HotkeyLabel{
                    frame={l=26, t=0},
                    label='Hide',
                    key='CUSTOM_H',
                    on_activate=function() self:apply_stock_action('hidden') end,
                    auto_width=true,
                },
                widgets.HotkeyLabel{
                    frame={l=34, t=0},
                    label='Zoom',
                    key='CUSTOM_Z',
                    on_activate=function() self:zoom_to_item() end,
                    auto_width=true,
                },
                widgets.HotkeyLabel{
                    frame={l=42, t=0},
                    label='View',
                    key='CUSTOM_V',
                    on_activate=function() self:view_item() end,
                    auto_width=true,
                },
                widgets.HotkeyLabel{
                    frame={l=50, t=0},
...
function LuaTrade:get_selected_items(one_only)
    local choices = self.subviews.list:getVisibleChoices()
    local selected_items = {}
    for _, choice in ipairs(choices) do
        if not choice.data.is_group then
            local flag = self.item_flags[choice.data.list_idx][choice.data.item_idx]
            if flag.selected then
                table.insert(selected_items, choice.data.item)
            end
        else
            for _, item_choice in ipairs(choice.data.items) do
                local flag = self.item_flags[item_choice.data.list_idx][item_choice.data.item_idx]
                if flag.selected then
                    table.insert(selected_items, item_choice.data.item)
                end
            end
        end
    end
    ...
    if one_only and #selected_items > 1 then
...
function LuaTrade:zoom_to_item()
    local items = self:get_selected_items(true)
    if #items == 0 then return end
    local item = items[1]
    dfhack.gui.revealInDwarfmodeMap(item.pos, true)
    dfhack.gui.setSelectedCoords(item.pos)
end

function LuaTrade:view_item()
    local items = self:get_selected_items(true)
    if #items == 0 then return end
    local item = items[1]
    dfhack.gui.revealInDwarfmodeMap(item.pos, true)
...
function LuaTrade:get_total_selected_value()
    ...
    local goods = self.items[list_idx]
    ...
    for i, item in ipairs(goods) do
        if goodflags[i].selected then
            total = total + common.get_perceived_value(item, mer)
        end
    end
    return total
end
...
function LuaTrade:cache_choices(list_idx, trade_bins)
    ...
    for item_idx, item in ipairs(self.items[list_idx]) do
        ...
        local is_requested = trade.open and dfhack.items.isRequestedTradeGood(item, mer) or false
        ...
        local data = {
            desc=desc,
            value=common.get_perceived_value(item, mer),
            list_idx=list_idx,
            item=item,
            item_idx=item_idx,
...
function LuaTrade:aggregate_choices(flat_choices, filter_str)
    ...
    for _, choice in ipairs(flat_choices) do
        ...
        if match then
            ...
            if self.item_flags[d.list_idx][d.item_idx].selected then
    ...
    for _, key in ipairs(order) do
        ...
        choice.icon = function() 
            local sel = 0
            for _, c in ipairs(g.items) do
                if self.item_flags[c.data.list_idx][c.data.item_idx].selected then
                    sel = sel + 1
                end
            end
...
function LuaTrade:toggle_group(choice, target_value)
    if target_value == nil then
        local target = true
        for _, item_choice in ipairs(choice.data.items) do
            local goodflag = self.item_flags[item_choice.data.list_idx][item_choice.data.item_idx]
    ...
    for _, item_choice in ipairs(choice.data.items) do
        self:toggle_item_base_internal(item_choice, target_value)
    end
end
...
function LuaTrade:toggle_range(idx, choice)
    ...
    local function choice_is_fully_selected(current_choice)
        ...
        local goodflag = self.item_flags[current_choice.data.list_idx][current_choice.data.item_idx]
    ...
    for i = list_idx, self.prev_list_idx, list_idx < self.prev_list_idx and 1 or -1 do
        ...
        if current_choice then
            self:toggle_item_base_internal(current_choice, target_value)
        end
    end
    self.prev_list_idx = list_idx
end
...
function LuaTrade:toggle_group_dry(choice, target_value, dry_run)
    if target_value == nil then
...
function LuaTrade:toggle_item_base(choice, target_value, dry_run)
    if choice.data.is_group then
        return self:toggle_group_dry(choice, target_value, dry_run)
    else
        local goodflag = self.item_flags[choice.data.list_idx][choice.data.item_idx]
        if target_value == nil then
            target_value = not goodflag.selected
        ...
        if dry_run then return target_value end
        return self:toggle_item_base_internal(choice, target_value)
    end
end
...
function LuaTrade:toggle_visible()
    ...
    for _, item_choice in ipairs(all_items) do
        if not self.item_flags[item_choice.data.list_idx][item_choice.data.item_idx].selected then
            target_value = true
            break
        end
    end
