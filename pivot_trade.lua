...
local pivot_ui = reqscript('internal/pivot_trade/pivot_trade_ui')
...
function main()
    ...
    -- DEBUG: Check if the module actually contains PivotScreen
    if not pivot_ui or not pivot_ui.PivotScreen then
        print("Error: pivot_ui.PivotScreen is nil!")
        -- Try a "soft reload" if it's missing
        pivot_ui = reqscript('internal/pivot_trade/pivot_trade_ui')
    end

    
    -- Modern ZScreen show logic
    local pscreen = pivot_ui.PivotScreen{items = items_to_show}
    pscreen:show()
    -- pivot_ui.PivotScreen{items = items_to_show}:show()
end
